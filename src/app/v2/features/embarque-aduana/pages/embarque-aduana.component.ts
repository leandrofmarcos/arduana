...
@Component({
  ...
  styles: [
    ...
    .tl-dot { width:28px; height:28px; border-radius:50%; border:3px solid var(--color-border); background:#fff; display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; cursor:default; }
    ...
    .tl-label { font-size:10px; margin-top:4px; white-space:nowrap; color:var(--color-text-muted); }
  ...
  template: `
    <div class="container-standard">
      ...
      <div class="dashboard-header">
        ...
        <button class="btn btn-primary" *ngIf="mode !== 'form'" (click)="openForm()">+ Novo Embarque</button>
      </div>
      ...
      <ng-container *ngIf="mode === 'detail' && detail">
        ...
        <div class="detail-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btn-secondary" (click)="abrirModalStatus()">🔄 Alterar Status</button>
            <button class="btn btn-secondary" (click)="openForm(detail)">✏️ Editar</button>
            <button class="btn btn-secondary" (click)="mode = 'list'">← Voltar</button>
          </div>
        </div>
        ...
        <div class="detail-grid">
          ...
          <div class="content-section">
            ...
            <ul class="info-list">
              <li><span class="lbl">Ref. Ominium</span><span class="val">{{ detail.refOminium || '—' }}</span></li>
              <li><span class="lbl">IMP</span><span class="val">{{ detail.imp || '—' }}</span></li>
              <li><span class="lbl">BL</span><span class="val">{{ detail.bl || '—' }}</span></li>
              <li><span class="lbl">Container</span><span class="val">{{ detail.container || '—' }}</span></li>
              <li><span class="lbl">LI</span><span class="val">{{ detail.li || '—' }}</span></li>
              <li><span class="lbl">Registro</span><span class="val">{{ detail.registro || '—' }}</span></li>
              <li><span class="lbl">Ref. Agente</span><span class="val">{{ detail.refAg || '—' }}</span></li>
              <li><span class="lbl">KG</span><span class="val">{{ detail.kg | number:'1.3-3' }}</span></li>
              <li *ngIf="detail.custoDespachanteId"><span class="lbl">Custo Base</span><span class="val cod-badge">{{ nomeCusto(detail.custoDespachanteId) }}</span></li>
              <li *ngIf="detail.orcamentoVendaId"><span class="lbl">Orçamento</span><span class="val cod-badge">{{ nomeOrc(detail.orcamentoVendaId) }}</span></li>
              <li *ngIf="detail.observacao"><span class="lbl">Observação</span><span class="val" style="text-align:right">{{ detail.observacao }}</span></li>
            </ul>
          </div>
          ...
          <div class="content-section">
            ...
            <ul class="info-list">
              <li><span class="lbl">Porto Origem</span><span class="val">{{ nome('portoOrigem', detail.portoOrigemId) }}</span></li>
              <li><span class="lbl">Porto Destino</span><span class="val">{{ nome('portoDestino', detail.portoDestinoId) }}</span></li>
              <li><span class="lbl">Navio</span><span class="val">{{ nome('navio', detail.controleNavioId) }}</span></li>
              <li><span class="lbl">Despachante</span><span class="val">{{ nome('despachante', detail.despachanteId) }}</span></li>
              <li><span class="lbl">Agente de Carga</span><span class="val">{{ nome('agente', detail.agenteCargaId) }}</span></li>
              <li *ngIf="detail.exportadorId"><span class="lbl">Exportador</span><span class="val">{{ nome('exportador', detail.exportadorId) }}</span></li>
              <li><span class="lbl">ETD</span><span class="val">{{ detail.etd | date:'dd/MM/yyyy' }}</span></li>
              <li><span class="lbl">ETA</span><span class="val">{{ detail.eta | date:'dd/MM/yyyy' }}</span></li>
              <li *ngIf="detail.avisoPrevisao"><span class="lbl">Aviso Previsão</span><span class="val">{{ detail.avisoPrevisao | date:'dd/MM/yyyy' }}</span></li>
              <li *ngIf="detail.avisoChegada"><span class="lbl">Aviso Chegada</span><span class="val">{{ detail.avisoChegada | date:'dd/MM/yyyy' }}</span></li>
              <li *ngIf="detail.dataRegistro"><span class="lbl">Data Registro</span><span class="val">{{ detail.dataRegistro | date:'dd/MM/yyyy' }}</span></li>
              <li *ngIf="detail.desemb"><span class="lbl">Desembaraço</span><span class="val">{{ detail.desemb | date:'dd/MM/yyyy' }}</span></li>
              <li *ngIf="detail.entrega"><span class="lbl">Entrega</span><span class="val">{{ detail.entrega | date:'dd/MM/yyyy' }}</span></li>
            </ul>
          </div>
        </div>
...
export class EmbarqueAduanaComponent implements OnInit {
