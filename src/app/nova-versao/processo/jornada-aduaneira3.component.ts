...
@Component({
  standalone: true,
  selector: 'app-jornada-aduaneira3-nova',
  imports: [CommonModule, RouterModule],
  template: `
    <div class="card">
      <div class="header">
        <div class="title"><span class="pkg">📦</span>Jornada Aduaneira 3</div>
  ...
  `,
  styles: [
    `:host{display:block;background:linear-gradient(135deg,#667eea20 0%,#764ba220 100%),var(--color-bg)}`,
    `.card{background:transparent;border-radius:12px;padding:24px}`,
    `.header{display:flex;flex-direction:column;margin-bottom:8px}`,
    `.title{display:flex;align-items:center;gap:10px;font-size:clamp(1.6rem,2.4vw,2rem);font-weight:800;color:var(--color-text)}`,
    ...
    `.board{position:relative;width:100%;max-width:1200px;height:clamp(420px,calc(100vh - 220px),640px);margin:0 auto;overflow:hidden;background:linear-gradient(135deg,#667eea20 0%,#764ba220 100%),var(--color-bg);border-radius:12px;--circle-size:clamp(72px,10vh,110px)}`,
    ...
    `.panel{margin-top:12px;background:transparent;border-radius:12px}`,
    `.panel-header{margin-top:12px}`,
    `.panel-inner{padding:8px 12px;display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:8px}`,
    ...
    `.info-desc{color:var(--color-text-secondary);margin-bottom:8px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}`,
    ...
    ,`.divider{height:2px;background:var(--gradient-primary);opacity:.7;border-radius:999px;margin:12px 0}`
  ]
})
export class JornadaAduaneiraNova3Component {
  steps = [
    { id: 0, title: 'Processo', subtitle: 'Cadastro', description: 'Dados do processo e documentos', icon: '🧩', color: 'blue', position: { x: 60, y: 460 } },
    { id: 1, title: 'Packlist', subtitle: 'Volumes', description: 'Itens e volumes do embarque', icon: '📦', color: 'cyan', position: { x: 240, y: 460 } },
    { id: 2, title: 'Custo', subtitle: 'Planilha', description: 'Cálculo de custos e despesas', icon: '🧮', color: 'purple', position: { x: 420, y: 420 } },
    { id: 3, title: 'Venda', subtitle: 'Proposta', description: 'Condições comerciais e aprovação', icon: '💼', color: 'indigo', position: { x: 600, y: 360 } },
    { id: 4, title: 'Aduana', subtitle: 'Registros', description: 'Conferência e trâmites aduaneiros', icon: '🛃', color: 'orange', position: { x: 780, y: 300 } },
    { id: 5, title: 'Numerário', subtitle: 'Pagamentos', description: 'Controle de pagamentos e taxas', icon: '💵', color: 'green', position: { x: 960, y: 220 } },
    { id: 6, title: 'Fechamento', subtitle: 'Conclusão', description: 'Finalização e liberação', icon: '✅', color: 'emerald', position: { x: 1080, y: 120 } }
  ];
  currentStep = 0;
  animateToken = false;
